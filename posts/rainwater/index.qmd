...
The initial place I looked was MLS (Zillow). The [Indiana Association of Realtors](https://data.indianarealtors.com/) has couple great resources. Unfortunately, these sources only include Residential property. And, depending on the county, ~50% or more of net due tax comes from Commercial, Industrial, and Aggricultural property. Here's Adams county:
...
I grabbed the states property tax spending by county [here](https://gateway.ifionline.org/public/pts/pts-levies-detail.aspx). And the total county disbursements [here](https://gateway.ifionline.org/public/download.aspx).

I've made the code used to aggregate this data publically [available](https://drive.google.com/file/d/1lzvu8UqMkSsXCP5ij5ub9QGQhrChEmkf/view?usp=sharing), and will provide the data at request (because it was annoying to get).
