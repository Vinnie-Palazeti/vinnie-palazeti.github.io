...
I decided to take a look. I use the suggested variables from Darling, Berger, & Zucman to model the Michigan Survey Consumer Index.

I have not seen an index or measure from Will, so I am not sure how to test his hypothesis.
...
all_bls = bls_series(series_dict, url, date_range)
...
<!--  
Maybe we are only interested in the Expectation, but it is funny to look at the inner workings of the model. 
...
Including the Real Income reduces the residual by about 15%, but it does not close the gap entirely. So, the opposing view does not totally explain the divergence. 
...
From this modeling exercise, it looks like economic shocks can result in large residuals. 
